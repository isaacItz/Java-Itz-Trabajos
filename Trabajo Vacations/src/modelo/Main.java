...
public class Main {
		...
		do {
			...
			case 2:
				...
				if (hayDatos()) {
					...
					escribir("no hay datos registrados", 0);
				break;
			...
			case 3:// numeros perfectos
				if (hayDatos()) {
					...
					if (!perfectos.isEmpty())
						escribir(perfectos, 1);
					else
						escribir("no hay numeros perfectos", 2);
					if (!continuar("desea hacer mas operaicones?"))
						System.exit(0);
				} else
					escribir("no hay datos registrados", 0);
			...
			case 4:
				...
				if (hayDatos()) {
					...
					if (!termina7.isEmpty())
						...
						escribir("no hay numeros que terminen en 7", 2);
					if (!continuar("desea hacer mas operaicones?"))
						System.exit(0);
				} else
					escribir("no hay datos registrados", 0);
			...
			case 5:
				...
				if (hayDatos()) {
					...
					if (!factoriales.isEmpty())
						escribir(factoriales, 1);
					else
						escribir("no hay numeros que terminen en 3", 2);
					if (!continuar("desea hacer mas operaicones?"))
						System.exit(0);
				} else
					escribir("no hay datos registrados", 0);
			...
			case 6:
				...
				if (hayDatos()) {
					...
					if (suma == 0)
						escribir("no hay datos", 0);
					else
						escribir("la sumatoria de los numeros primos es " + suma, 1);
			...
			case 9:
				...
				if (numeros.size() > 1) {
					...
					if (numeros.size() % 2 == 0) {
						escribir("la mediana es: " + numeros.get(div - 1) + " y " + numeros.get(div), 1);
					} else
						escribir("la media es: " + numeros.get(div), 1);

					if (!continuar("desea hacer mas operaicones?"))
						System.exit(0);
				} else
					escribir("ingresa mas datos para realizar esta operacion", 2);
			...
			default:
				escribir("opcion no disponible", 0);
				break;
			}
	...
	private static void llenarLista() {
		...
		do {
			...
			if (!numeros.contains(n)) {
				...
				escribir("El Numero " + n + " ya ha Sido Guardado Anteriormente", 0);
	...
	private static void escribir(Object mensaje, int tipo) {
